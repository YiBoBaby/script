...
//自动生成key值
func (c *HashRing) generateKey(element string, index int) string {
	//副本key生成逻辑
...
func (c *HashRing) hashKey(key string) uint32 {
	if len(key) < 64 {
		...
		return crc32.ChecksumIEEE(srcArr[:len(key)])
	}
...
//向hash环中添加节点
func (c *HashRing) Add(element string) {
	//加锁
	c.Lock()
	//解锁
	defer c.Unlock()
...
//删除节点
func (c *HashRing) remove(element string) {
	for i := 0; i < c.virtualNode; i++ {
...
//顺时针查找最近的节点
func (c *HashRing) search(key uint32) int {
	//查找算法
	f := func(x int) bool {
